...
* **Problema:** O site dependia de requisições externas para carregar a família de fontes Playfair Display via Google Fonts, o que introduzia latência e dependência de rede, além de violar a escolha do cliente por não utilizar dependências externas.
* **Correção:** As fontes Playfair Display foram removidas do Google Fonts e todas as instâncias no CSS (`.main-title-bg`, `.mini-texto`, `.faq-question`, `.faq-answer-content`, `.faq-title`, `.container-sobre`) foram substituídas pela família **Montserrat** (já local), com ajustes finos de `font-weight` e `font-size` para manter a hierarquia visual original sem o peso adicional das fontes externas.
* **Por que importa:** Elimina latência de rede e dependência de servidor externo, fortalece a privacidade e autonomia do site, e simplifica o carregamento total para ~350 KB (HTML+CSS+JS), garantindo consistência visual em qualquer conexão e conformidade com a escolha do cliente por não utilizar APIs de terceiros.
